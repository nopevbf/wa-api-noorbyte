...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    const addLog = (colorClass, label, text, delay = 100) => {
    ...
    async function tarikDataDparagon() {
        const dpApiUrl = document.getElementById('dpApiUrl').value.replace(/\/$/, "");
        const dpEmail = document.getElementById('dpEmail').value;
        const dpPassword = document.getElementById('accountPassword').value;

        if (!dpApiUrl || !dpEmail || !dpPassword) throw new Error("Kredensial DParagon belum lengkap.");

        // [STEP 1] LOGIN
        await addLog('text-blue-400', 'STEP 1', 'Melakukan login ke sistem DParagon...', 100);
        ...
        let dpToken = authData.access_token || authData.data?.access_token || authData.payload?.access_token;
        if (!dpToken) throw new Error('Gagal mendapatkan access_token dari response.');
        await addLog('text-emerald-400', 'SUCCESS', 'Login berhasil, access_token didapatkan.', 300);

        // [STEP 2] ON PROGRESS TASK
        await addLog('text-blue-400', 'STEP 2', 'Mengambil data on-progress task...', 500);
        ...
        if (!taskRes.ok) throw new Error(`Gagal fetch on-progress task (HTTP ${taskRes.status})`);
        const taskData = await taskRes.json();
        const payloadData = taskData.payload || [];
        if (payloadData.length === 0) throw new Error('Data payload kosong atau tidak ditemukan.');
        
        const tasksList = payloadData.map(task => ({
            dates: `${task.start_date || ''} - ${task.end_date || ''}`,
            task_description: task.task_description || ''
        }));
        await addLog('text-emerald-400', 'SUCCESS', `Data task berhasil diekstrak (${tasksList.length} task).`, 300);

        // [STEP 3] POST NEW TASK
        await addLog('text-blue-400', 'STEP 3', 'Melakukan POST data ke /daily-reports/new-task...', 500);
        const now = new Date();
        ...
        if (!postTaskRes.ok) throw new Error(`Gagal POST new-task (HTTP ${postTaskRes.status})`);
        await addLog('text-emerald-400', 'SUCCESS', 'POST /new-task berhasil dikirim ke server!', 300);

        // [STEP 4] GET REPORT CODE
        await addLog('text-blue-400', 'STEP 4', 'Mengambil daily_report_code dari list terbaru...', 500);
        const listRes = await fetch(`${dpApiUrl}/daily-reports/list?dates=&employee_position_id=`, {
            headers: { 'Authorization': `Bearer ${dpToken}`, 'Content-Type': 'application/json' }
        });
        if (!listRes.ok) throw new Error(`Gagal ambil list report (HTTP ${listRes.status})`);
        const listData = await listRes.json();

        const group = listData.payload?.group || {};
        const keys = Object.keys(group);
        if (keys.length === 0) throw new Error('Tidak ada data report ditemukan (Keys length 0).');
        ...
        await addLog('text-emerald-400', 'SUCCESS', `Berhasil menyimpan code: ${reportCode}`, 300);

        // [STEP 5] GET SUMMARY REPORT
        await addLog('text-blue-400', 'STEP 5', `Mengambil detail laporan...`, 500);
        const summaryRes = await fetch(`${dpApiUrl}/daily-reports/summary-daily-report?code=${reportCode}`, {
            headers: { 'Authorization': `Bearer ${dpToken}`, 'Content-Type': 'application/json' }
        });
        if (!summaryRes.ok) throw new Error(`Gagal ambil summary (HTTP ${summaryRes.status})`);
        const summaryData = await summaryRes.json();

        let rawMessage = null;
        if (summaryData.payload?.message) rawMessage = summaryData.payload.message;
        else if (typeof summaryData.payload === 'string') rawMessage = summaryData.payload;
        else if (summaryData.data?.message) rawMessage = summaryData.data.message;
        else if (summaryData.message && summaryData.message.length > 50) rawMessage = summaryData.message;

        if (!rawMessage) throw new Error('Message laporan tidak ditemukan atau kosong!');
        await addLog('text-emerald-400', 'SUCCESS', 'Payload message berhasil diekstrak.', 300);
    ...
    async function kirimKeWhatsApp(pesan) {
        ...
        if (!apiKey || !targetNumber) throw new Error("Device WA atau Target Nomor kosong.");
        if (!pesan) throw new Error("Pesan kosong, jalankan proses fetch data terlebih dahulu.");

        await addLog('text-amber-400', 'STEP 6', `Mengirim pesan ke WhatsApp (${targetNumber})...`, 500);
        const waRes = await fetch(`${API_URL}/send-message`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${apiKey}` },
            body: JSON.stringify({ number: targetNumber, message: pesan, msg_type: 'text' })
        });

        const waResult = await waRes.json();
        if (waRes.ok && waResult.status === 'success') {
            await addLog('text-emerald-400', 'SUCCESS', `Laporan terkirim! (ID: ${waResult.data?.message_id})`, 300);
        } else {
    ...
    function updatePreviewUI(isActive) {
        ...
        if (isActive) {
            ...
            const isWeekdays = document.getElementById('freqWeekdays').checked ? ' (Hari Kerja)' : '';
...
Mendeteksi waktu berjalan...`;
            ...
            msgPreview.innerText = cachedMessage || `🔔 DAILY REPORT - DPARAGON
...
(Sistem otomatisasi MATI. Klik Run Automation Now untuk manual)`;
    ...
    function startScheduler() { 
        ...
        addLog('text-purple-400', 'SCHEDULER', 'Jadwal Otomatis AKTIF. Memantau waktu...', 0);
        ...
        scheduleInterval = setInterval(async () => {
            ...
            if (currentHourMin === fetchTimeSetting && !window.hasFetchedForMinute) {
                ...
                await addLog('text-purple-400', 'SCHEDULER', `Waktu Fetch Data tiba (${fetchTimeSetting}). Memulai proses...`);
                try { await tarikDataDparagon(); } catch (e) { await addLog('text-red-500', 'SCHEDULER ERROR', `Gagal fetch data: ${e.message}`); }
            ...
            if (currentHourMin === sendWaTimeSetting && !window.hasSentWaForMinute) {
                ...
                await addLog('text-purple-400', 'SCHEDULER', `Waktu Send WA tiba (${sendWaTimeSetting}). Memulai pengiriman...`);
                try { await kirimKeWhatsApp(cachedMessage); } catch (e) { await addLog('text-red-500', 'SCHEDULER ERROR', `Gagal kirim WA: ${e.message}`); }
    ...
    function loadSavedSettings() {
        ...
        if (savedSettings) {
            if (document.getElementById('dpApiUrl')) document.getElementById('dpApiUrl').value = savedSettings.dpApiUrl || '';
            if (document.getElementById('dpEmail')) document.getElementById('dpEmail').value = savedSettings.dpEmail || '';
            if (document.getElementById('accountPassword')) document.getElementById('accountPassword').value = savedSettings.dpPassword || '';
            
            if (document.getElementById('scheduleToggle')) document.getElementById('scheduleToggle').checked = savedSettings.scheduleEnabled || false;
            if (document.getElementById('fetchTime')) document.getElementById('fetchTime').value = savedSettings.fetchTime || '08:00';
            if (document.getElementById('sendWaTime')) document.getElementById('sendWaTime').value = savedSettings.sendWaTime || '09:00';
            
            if (savedSettings.frequency === 'weekdays') document.getElementById('freqWeekdays').checked = true;
            else document.getElementById('freqDaily').checked = true;

            if (document.getElementById('targetNumber')) document.getElementById('targetNumber').value = savedSettings.targetNumber || '';
    ...
    if (btnSaveSettings) {
        btnSaveSettings.addEventListener('click', () => {
            ...
            setTimeout(() => {
                ...
                showModal('Pengaturan Disimpan', 'Konfigurasi jadwal dan API berhasil disimpan.');
