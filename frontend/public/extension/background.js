...
function captureAndNext(state, results, preCapturedDataUrl = undefined) {
    const processResult = (dataUrl) => {
        ...
        chrome.alarms.create('next_process', { delayInMinutes: 0.5 });
    };
...
async function processNext() {
    ...
    if (!queue || queue.length === 0) {
        ...
        sendLog(`🔥 Pulse LCR Pro: Misi Selesai!`, 'success');
        broadcastToUI('PULSE_EXT_DONE', {});
        return;
    }

    let nextUrl = queue.shift(); if (nextUrl.includes('instagram.com')) { nextUrl = nextUrl.replace(/instagram\.com\/reels?\//i, 'instagram.com/p/'); }
